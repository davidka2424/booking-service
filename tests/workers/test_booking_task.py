...
def make_booking(status: BookingStatus = BookingStatus.PENDING) -> MagicMock:
    ...
    booking.scheduled_at = datetime.now(tz=timezone.utc) + timedelta(days=1)
    return booking


async def _call_task(booking_id: str) -> dict:
    """Helper: call task's underlying function directly."""
...
class TestConfirmBookingTask:
    """Unit tests for confirm_booking task logic."""

    async def test_task_success_path(self) -> None:
        """Task should mark booking as CONFIRMED when no simulated failure."""
        ...
        with patch("app.workers.tasks.booking.get_session_factory", return_value=mock_factory), \
             patch("app.workers.tasks.booking.BookingRepository", return_value=mock_repo), \
    ...
    async def test_task_failure_path(self) -> None:
        """Task should mark booking as FAILED when simulated failure triggers."""
        booking = make_booking(BookingStatus.PENDING)
        mock_repo = AsyncMock()
        mock_repo.get_by_id.return_value = booking
        mock_repo.update_status.return_value = booking
    ...
    async def test_task_idempotency_already_confirmed(self) -> None:
        """Task called on already CONFIRMED booking should skip without side effects."""
    ...
    async def test_task_idempotency_already_cancelled(self) -> None:
        """Task called on CANCELLED booking should skip."""
    ...
    async def test_task_booking_not_found(self) -> None:
        """Task called with non-existent booking_id should return skipped."""
        mock_repo = AsyncMock()
    ...
    async def test_mock_notification_is_called_on_success(self) -> None:
        """On success path, mock notification function must be invoked."""
        booking = make_booking(BookingStatus.PENDING)
        mock_repo = AsyncMock()
        mock_repo.get_by_id.return_value = booking
        mock_repo.update_status.return_value = booking
        ...
        with patch("app.workers.tasks.booking.get_session_factory", return_value=mock_factory), \
             patch("app.workers.tasks.booking.BookingRepository", return_value=mock_repo), \
             patch("app.workers.tasks.booking.random.random", return_value=0.99), \
             patch("app.workers.tasks.booking._mock_external_notification", mock_notify):
            await _call_task(str(booking.id))
